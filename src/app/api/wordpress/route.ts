...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const res = await fetch(`${wpBase}/wp-json/custom/v1/vehicles-photo-status`);
    if (!res.ok) throw new Error('Failed to fetch');
    const allVehicles = await res.json(); // [{ id, title, status, date, manage_id, has_photo }, ...]

    // 写真がない車両だけを抽出
    const vehiclesWithoutPhoto = allVehicles
        .filter((v: any) => !v.has_photo)
        .map((v: any) => ({
            id: String(v.id),
            title: v.title,
            status: v.status,
            date: v.date,
            v_manage_id: v.manage_id,
        }));
