...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    do {
      const res = await fetch(`${wpBase}/wp-json/wp/v2/inventory?per_page=${perPage}&page=${page}&_embed`);
      if (!res.ok) break;
    ...
    const vehicles = await Promise.all(allVehicles.map(async (v: any) => {
      ...
      try {
        ...
        if (mr.ok) { const md = await mr.json(); hasPhoto = Array.isArray(md) && md.length > 0; }
      } catch {}
      return {
        id: String(v.id),
        ...
        ...(v.acf && typeof v.acf === 'object' ? v.acf : {}),
        has_photo: hasPhoto,
      };
    }));
